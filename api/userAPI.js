...
userRouter.post("/register",async(req,res)=>{
  ...
  role=role||"jobseeker";
  let existingUser=await usersSchema.findOne({email});
  if(existingUser){
    return res.status(409).json({success:false,message:"Email already registered"});
  }
  let hashedPassword=await bcrypt.hash(password,10);
  let newUserDoc=await usersSchema.create({
    name,
    email,
...
userRouter.post("/login",async(req,res)=>{
  ...
  let user=await usersSchema.findOne({email}).select("+password");
  if(!user){
...
userRouter.put("/profile",verifyToken,async(req,res)=>{
  ...
  let updatedUser=await usersSchema.findByIdAndUpdate(
    req.user._id,
    {$set:{...modifiedProfile}},
